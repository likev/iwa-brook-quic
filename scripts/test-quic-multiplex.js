/**
 * Test multi-stream capability on a single QUIC connection against Brook server
 */
...
async function test() {
  ...
  // Let's test stream 0 (client bidi stream 0)
  // And stream 4 (client bidi stream 1)
  console.log('\n--- Testing Stream 0 ---');
  await sendBrookRequest(quic, streamHandlers, 0, 'example.com', 80, PASSWORD);

  console.log('\n--- Testing Stream 4 on SAME QUIC Connection ---');
  try {
    await sendBrookRequest(quic, streamHandlers, 4, 'example.com', 80, PASSWORD);
    console.log('Stream 4 succeeded on the SAME QUIC connection!');
  } catch (err) {
...
async function sendBrookRequest(quic, streamHandlers, streamId, host, port, password) {
  return new Promise((resolve, reject) => {
    ...
    streamHandlers.set(streamId, {
      onData: (data, fin) => {
        ...
        while (serverHandshakeDone) {
          ...
          console.log(`Stream ${streamId} received decrypted payload (${plain.length}B):`, new TextDecoder().decode(plain).slice(0, 50));
          clearTimeout(timer);
          resolve();
          return;
        }
    ...
    // Build dstBytes: [0x03, domainLen, domain..., port_hi, port_lo]
    ...
    quic.sendStream(streamId, sealFrame(ck, cnCopy, req), false);
  });
}
